...
precoProd=[20.5, 1.0, 4.0]                                                    # listas globais para o uso do programa
quantProd=[2, 4, 20 ]
...
def relatorio():
            ...
            print(f"|\033[;32m{codigoProd}\033[m")
            lin2()
...
def cadastrar():   #função de cadastrar um produto
    ...
    while True:
        ...
        if valor != -1:  
            print(f"\033[0;31mO código \"{prod.codigo}\" já está cadastrado!\033[m")
        else:
                if prod.codigo <= 0:
                    print("\033[0;31mInsira um número válido para o cadastramento!\033[m")
                else:
                    break
    ...
    while True:
        ...
        if valor != -1:
            print(f"\033[0;31mO nome \"{prod.nome}\" já está cadastrado\033[m ")
        else:
            break
...
def atualizarProd():    #função para atualizar o produto
    while True:
        codigoProduto = int(input("Insira o Código do produto que deseja alterar ou -9 para sair: "))
        if codigoProduto == -9:
            return
    ...
    while True:    
        ...
        if precoNovo <= 0:
            print("\033[1;31mValor invalido, insira um valor maior que zero!\033[m")  
        else:
            break
    while True:
        ...
        for i in range(len(codigoProd)):
            if codigoProduto == -1:
                ...
                if codigoProduto == codigoProd[i]:
                    lin2()
                    print('|código|',codigoProd[i],'nome|', nomeProd[i],'|preço|','R$', precoProd[i],'|Quantidade|', quantProd[i],'|')
                    lin2()
...
def registrarCompra():
    ...
    compraQuant=[ ]
    while True:
        codigoProduto = int(input("Insira o Código do produto que deseja \033[1;30;40mCOMPRAR:\033[m ou -9 para sair: "))#\033[m é uma função de mudar cor!
        pos = verif_posicao(codigoProduto)
        if codigoProduto == -9:
            break
        if pos == -1:
            ...
            if quantidadeUser <= 0:
                print("\033[31mQuantidade inválida! Selecione uma quantidade maior que 0!\033[m")
            else:
                q = quantProd[pos] - quantidadeUser
                if q <= -1:
                    print("\033[1;31m\nQuantidade superior a quantidade em estoque!\033[m")        
                    print(f"\033[1;33mTemos {quantProd[pos]} em estoque.\033[m")
                else:
    ...
    while True:
        if quantidadeUser <=0:
            ...
            for i in range(len(compraCodigo)):
                print(f"| {compraCodigo[i]:<7}{compraNome[i]:<10}       {compraPreco[i]:<10}      {compraQuant[i]:<3}            {compraPreco[i] * compraQuant[i]}|")
